...
function Locator(route, options) {
  ...
  if (spacing === 'acceldecel') {
    ...
    if (version === 'v5') {
      for (i = 0; i < route[0].legs[0].steps.length; i++) {
        timeCounter = timeCounter + route[0].legs[0].steps[i].duration * 1000;
        times.push(timeCounter);
        coordinates.push(route[0].legs[0].steps[i].maneuver.location);
      }
    } else if (version === 'v4') {
      for (i = 0; i < route.steps.length; i++) {
        timeCounter = timeCounter + route.steps[i].duration * 1000;
        times.push(timeCounter);
        coordinates.push(route.steps[i].maneuver.location.coordinates);
      }
    }
