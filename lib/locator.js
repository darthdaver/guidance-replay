 ...
 *
 * @class
 * @param {Object} route Response from Mapbox Directions API.
 */
function Locator(route, options) {
  var directions = route.routes;
  ...
  if (spacing === 'acceldecel') {
    ...
    if (version === 'v5') {
      for (i = 0; i < directions[0].legs[0].steps.length; i++) {
        timeCounter = timeCounter + directions[0].legs[0].steps[i].duration * 1000;
        times.push(timeCounter);
        coordinates.push(directions[0].legs[0].steps[i].maneuver.location);
      }
    } else if (version === 'v4') {
      for (i = 0; i < directions.steps.length; i++) {
        timeCounter = timeCounter + directions.steps[i].duration * 1000;
        times.push(timeCounter);
        coordinates.push(directions.steps[i].maneuver.location.coordinates);
      }
    }
