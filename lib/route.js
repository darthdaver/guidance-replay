...
function getSteps(route) {
  var v = util.version(route);
  var steps = [];
  if (v === 'v4') {
    var routeSteps = route[0].steps.slice(0);
  ...
  if (v === 'v5') {
    if (route[0].legs.length === 1) {
      steps = route.legs[0].steps;
    } else {
      steps = [];
      for (var i = 0; i < route[0].legs.length; i++) {
        steps = steps.concat(route[0].legs[i].steps); 
      }
    }
...
function interpolateConstant(routeStep) {
  ...
  for (var j = 0; j < intervals.length; j++) {
    intervals[j] = totalDist > 0 ? intervals[j] / totalDist : 0;
    totalTime += intervals[j] * routeStep.duration * 1000;
    times.push(Math.round(totalTime));
  }

  console.log(routeStep.duration * 1000)
  console.log(times)
  console.log(times[times.length-1])
  console.log(Math.abs(routeStep.duration * 1000 - times[times.length-1]))
