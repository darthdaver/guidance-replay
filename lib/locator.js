...
function Locator(directions, options) {
  ...
  if (spacing === 'acceldecel') {
    ...
    if (version === 'v5') {
      ...
      for (i = 0; i < directions[0].steps.length; i++) {
        timeCounter = timeCounter + directions[0].steps[i].duration * 1000;
        times.push(timeCounter);
        coordinates.push(directions[0].steps[i].maneuver.location.coordinates);
      }
    }
